 ...
 * This includes adding, editing, listing, and clearing records stored in sessionStorage.
 */

function persistRecords(records) {
    var recordsString = JSON.stringify(records);
    sessionStorage.setItem("tbRecords", recordsString);
    var storageMode = sessionStorage.getItem('storageMode');
    if (storageMode === 'local') {
        var user = JSON.parse(sessionStorage.getItem('user'));
        if (user && user.Email) {
            var userRecordsKey = 'tbRecords_' + user.Email;
            localStorage.setItem(userRecordsKey, recordsString);
        }
    }
}
...
// Function to compare dates for sorting records in descending order (most recent first)
...
// Deletes a record from sessionStorage
  ...
  if (tbRecords.length==0)
  {
    ...
    */
    ...
    if (storageMode === 'local') {
      var user = JSON.parse(sessionStorage.getItem('user'));
      if(user && user.Email) {
        localStorage.removeItem('tbRecords_' + user.Email);
      }
    }
  }
  else
  {
    persistRecords(tbRecords);
  }
}
...
// Function to add a record to sessionStorage.
function addRecord() {
  if (checkRecordForm()) {
    var record = {
    ...
    var tbRecords = sessionStorage.getItem("tbRecords") ? JSON.parse(sessionStorage.getItem("tbRecords")) : [];
...
function listRecords() {
  var tbRecords = sessionStorage.getItem("tbRecords") ? JSON.parse(sessionStorage.getItem("tbRecords")) : null;
  ...
  if (tbRecords != null && tbRecords.length > 0) {
    ...
    $("#tblRecords").html(
      ...
      " <tr>" +
      "   <th>Date</th>" +
      "   <th><abbr title='Thyroid Stimulating Hormone'>TSH<br>(mIU/l)</abbr></th>" +
      "   <th><abbr title='Thyroglobulin'>Tg<br>(ug/L)</abbr></th>" +
